...
\subsubsection*{L'outil de gestion de projet \logo{Trello}}

Sur les conseils de nos tuteurs, M. \propre{Demougin} et Mme. \propre{Pierrot}, nous avons adopté l’outil \logo{Trello}. Cet outil nous a permis de lister et d’organiser les différentes tâches à accomplir, tout en suivant leur état d'avancement. En collaboration avec nos tuteurs, nous avons ainsi identifié les fonctionnalités à implémenter et avons réparti leur développement au sein du groupe.

\subsubsection*{Le framework \logo{Symfony}}
...
Après avoir comparé Laravel et \logo{Symfony}, nous avons opté pour ce dernier, son architecture orientée objet étant plus familière et mieux adaptée à notre approche du développement.

\subsubsection*{Le moteur de template \logo{Twig}}

Pour générer dynamiquement les pages HTML, nous avons utilisé \logo{Twig}, le moteur de template recommandé par \logo{Symfony}. Ce choix a facilité la séparation entre la logique métier et l'affichage, améliorant ainsi la lisibilité et la maintenabilité du code.
...
Concernant les technologies, nous avons confirmé notre choix préliminaire des technologies et sommes restés avec \logo{Symfony} comme framework principal.

Il convient alors de vous présenter brièvement comment \logo{Symfony} fonctionne.

\subsubsection*{L'architecture \logo{MVC}}

Tout d'abord, tout code \logo{Symfony} est structuré en respectant l'architecture MVC, ce qui signifie que chaque élément que l'on va ajouter à notre projet est divisé en trois parties : le \textbf{modèle} (M), la \textbf{vue} (V) et le \textbf{contrôleur} (C). 
